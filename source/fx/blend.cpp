...
namespace detail
{
	...
	kueken::blend::name object::loadBlend
	(
		void * Pointer
	)
	{
		...
		{
			TiXmlAttribute* Attribute = Blend->FirstAttribute();
			do
			{
		...
		kueken::blend::name Name = Renderer->create(Creator);

		Blends.insert(std::make_pair(Id, Name));
