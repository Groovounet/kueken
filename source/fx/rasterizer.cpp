...
namespace detail
{
	...
	kueken::rasterizer::name object::loadRasterizer
	(
		void * Pointer
	)
	{
		...
		if(Rasterizer)
		{
			...
			do
			{
				...
				{
					atoi(Attribute->Value());
				}
				else if(!strcmp("type", Attribute->Name()))
				{
					if(!strcmp("polygon", Attribute->Value()))
						return detail::loadPolygon(*Renderer, Rasterizer, Id);
					else if(!strcmp("line", Attribute->Value()))
						return detail::loadLine(*Renderer, Rasterizer, Id);
					else if(!strcmp("point", Attribute->Value()))
						return detail::loadPoint(*Renderer, Rasterizer, Id);
					else if(!strcmp("discard", Attribute->Value()))
						return detail::loadDiscard(*Renderer, Rasterizer, Id);
					else
						assert(0);
				}
				else
	...
	)
	{
		...
		return loadRasterizer((void*)Root);
	}
/*
...
namespace rasterizer
{
	command::command() :
		Renderer(0),
		Rasterizer(kueken::rasterizer::name::Null())
	{}
